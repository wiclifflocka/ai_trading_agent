...
class AdvancedTradingStrategy:
    ...
    def _load_latest_model(self):
        try:
            ...
            if model_files and scaler_files:
                ...
                logger.info(f"Loaded existing model: {latest_model}")
        except Exception as e:
    ...
    def train_lstm_model(self):
        try:
            ...
            if not self.lstm_model:
                logger.warning("No working model available - using fallback strategy")
    ...
    def predict_price_movement(self, data: np.ndarray) -> Optional[float]:
        if self.lstm_model is None or self.scaler is None:
            logger.warning("No LSTM model available for prediction")
            return None
        try:
    ...
    def get_signal(self) -> float:
        try:
            ...
            if not orderbook or hist_data is None:
                logger.error("Failed to fetch market data for signal")
                return 0.0
    ...
    def adjust_threshold(self, volatility: float):
        if volatility is not None:
            ...
            logger.debug(f"Adjusted threshold to {self.threshold:.4f} based on volatility {volatility:.4f}")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if not orderbook or hist_data is None:
                    logger.error("Critical data missing - skipping iteration")
                ...
                if None in [volatility, current_price, balance]:
                    logger.error("Invalid indicator values - skipping iteration")
                    continue
                self.adjust_threshold(volatility)
                if self.position:
    ...
    def _execute_trade(self, side: str, size: float, price: float):
        try:
            ...
            logger.info(f"{side} position opened at {price}")
        except Exception as e:
